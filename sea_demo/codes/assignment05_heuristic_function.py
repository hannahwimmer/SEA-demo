...
def heuristic_score(schedule, 
    required_per_shift=2, penalties=[10000, 5000, 3000, 400, 300]):
    """
    ...
    if D >= 2:
        ...
        rest_violations = int(np.nansum(np.logical_and(np.nan_to_num(night_work), 
            np.nan_to_num(next_morning))))
    ...
    days_off_per_nurse = np.nansum(shifts_per_nurse_day == 0, axis=1)  # days off per nurse
    dayoff_violations = int(np.nansum(days_off_per_nurse == 0))      

    # sum up hard and soft penalties
    hard_penalty = (penalties[0] * coverage_deficit +
                    penalties[1] * rest_violations +
                    penalties[2] * daily_over)

    soft_penalty = (penalties[3] * fairness_penalty +
                    penalties[4] * dayoff_violations)
...
if __name__ == "__main__":
    max_iters = 100
    num_ants = 100
    tau = np.zeros((5,3,2)) # start with small dimensions (5 nurses, three days, 2 shifts), then scale up
